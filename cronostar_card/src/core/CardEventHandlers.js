...
export class CardEventHandlers {
  ...
  toggleMenu(e) {
    ...
    Logger.log(
      "UI",
      "[CronoStar] toggleMenu: isMenuOpen is now",
  ...
  async handleLanguageSelect(lang) {
    ...
    try {
      ...
      if (this.card.hass && profileName && presetType && globalPrefix) {
        ...
        const scheduleData = this.card.stateManager
          .getData()
          .map((p) => ({ time: p.time, value: p.value }));

        Logger.log(
          "LANG",
          `[CronoStar] Calling save_profile with: profileName='${profileName}', presetType='${presetType}', globalPrefix='${globalPrefix}', meta=`,
          { ...this.card.config.meta },
        );
        ...
        Logger.warn(
          "LANG",
          "Not enough info to save language preference (profileName, presetType, globalPrefix, or hass missing)",
        );
      }
    } catch (e) {
  ...
  async handleLoggingToggle(e) {
    ...
    this.card.config = {
      ...this.card.config,
      logging_enabled: newLoggingState,
    };
  ...
  handleDeleteSelected() {
    ...
    indices.forEach((idx) => {
      if (idx === 0 || idx === numPoints - 1) {
        Logger.warn(
          "UI",
          `Skipping deletion of boundary point at index ${idx}`,
        );
        return;
      }
  ...
  async handleCopyJson() {
    try {
      ...
      const lines = scheduleData.map(
        (p) => `{"time": "${p.time}", "value": ${p.value}}`,
      );
      const jsonFragment = `"schedule": [\n${lines.join(",\n")}\n]`;

      const successMsg = this.card.localizationManager.localize(
        this.card.language,
        "notify.json_copied",
      );
  ...
  async handleApplyNow() {
    ...
    if (!this.card.hass) {
      ...
      this.showNotification(
        localize("ui.apply_now_error") + " (HA not connected)",
        "error",
      );
      return;
    }
    if (!this.card.cronostarReady) {
      Logger.warn(
        "APPLY",
        "[CronoStar] Backend startup not completed yet, deferring apply.",
      );
    ...
    if (!targetEntity) {
      Logger.error(
        "APPLY",
        "[CronoStar] `target_entity` is not configured in the card.",
      );
      this.showNotification(
        localize("ui.apply_now_error") + ": target_entity not set",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const safeMeta = (() => {
        const src =
          this.card.config && typeof this.card.config === "object"
            ? this.card.config
            : {};
        const rest = { ...src };
        delete rest.entity_prefix;
        if (!rest.global_prefix && effectivePrefix)
          rest.global_prefix = effectivePrefix;
        // Ensure scheduler can apply immediately by persisting the canonical target entity in profile meta
        if (!rest.target_entity && targetEntity)
          rest.target_entity = targetEntity;
        // Persist drag snap config for frontend reference only (not used by backend schedule)
        if (this.card.config?.drag_snap)
          rest.drag_snap = this.card.config.drag_snap;
        return rest;
      })();
      ...
      await this.card.hass.callService(
        "cronostar",
        "apply_now",
        applyNowPayload,
      );
      ...
      setTimeout(() => {
        try {
          ...
          Logger.warn("APPLY", "[CronoStar] Error in setTimeout:", e);
        }
      }, 1000);
    } catch (err) {
      Logger.error("APPLY", '[CronoStar] Error during "Apply Now":', err);
      this.showNotification(
        `${localize("ui.apply_now_error")}: ${err.message}`,
        "error",
      );
    }
  ...
  async handleAddProfile() {
    ...
    try {
      if (!this.card.hass) {
        this.showNotification(
          localize("notify.add_profile_error", {
      ...
      if (
        Array.isArray(this.card.profileOptions) &&
        this.card.profileOptions.includes(profileName)
      ) {
        this.showNotification(
          localize("notify.add_profile_error", {
      ...
      try {
        ...
        if (!Array.isArray(this.card.profileOptions))
            ...
            try {
              ...
              if (st && !options.includes(profileName)) {
                ...
                if (attempts > 0) {
                  Logger.log(
                    "PROFILE",
                    `New profile '${profileName}' not yet in ${selectorEntity} options. Retrying in 1s... (${attempts} left)`,
                  );
              ...
              if (attempts > 0) {
                Logger.log(
                  "PROFILE",
                  `Error selecting ${profileName} on ${selectorEntity}, retrying... (${attempts} attempts left)`,
                );
                setTimeout(() => trySelect(attempts - 1), 1000);
              } else {
                Logger.warn(
                  "PROFILE",
                  `Failed to select new profile ${profileName} on ${selectorEntity} after retries:`,
                  e,
                );
              }
              /* v8 ignore stop */
      ...
      this.showNotification(
        localize("notify.add_profile_success", { "{profile}": profileName }),
        "success",
      );
    } catch (err) {
  ...
  async _fetchProfileNameSuggestions(presetType) {
    try {
      ...
      const existing = new Set(
        Array.isArray(this.card.profileOptions) ? this.card.profileOptions : [],
      );
      const currentPrefix = (this.card.config?.global_prefix || "").replace(
        /_+$/,
        "",
      );
      const names = new Set();
      ...
      files.forEach((f) => {
        ...
        const profs = Array.isArray(f?.profiles)
          ? f.profiles
          : Array.isArray(f?.profile_names)
            ? f.profile_names
            : [];
        profs.forEach((n) => {
  ...
  async _openAddProfileDialog() {
    ...
    const t = (key, fallback) => {
      try {
        const s = this.card.localizationManager.localize(
          this.card.language,
          key,
        );
    ...
    const presetType =
      this.card.selectedPreset || this.card.config?.preset_type || "thermostat";
    ...
    return new Promise((resolve) => {
      ...
      input.type = "text";
      input.placeholder = t(
        "prompt.add_profile_placeholder",
        "Enter profile name",
      );
      input.style.cssText =
        "width: 100%; padding: 10px; border-radius: 8px; border: 1px solid var(--divider-color); box-sizing: border-box;";

      const suggTitle = document.createElement("div");
      suggTitle.textContent = t(
        "prompt.add_profile_suggestions",
        "Suggestions from other cards",
      );
      suggTitle.style.cssText =
        "margin: 14px 0 8px 0; font-weight: 600; opacity: 0.8;";
      ...
      cancelBtn.style.cssText =
        "padding: 8px 12px; border-radius: 8px; border: 1px solid var(--divider-color); background: transparent; cursor: pointer;";
      const okBtn = document.createElement("button");
      okBtn.type = "button";
      okBtn.textContent = t("prompt.create", "Create");
      okBtn.style.cssText =
        "padding: 8px 12px; border-radius: 8px; border: none; background: var(--primary-color); color: white; cursor: pointer;";
  ...
  async handleDeleteProfile() {
    ...
    try {
      if (!this.card.hass) {
        this.showNotification(
          localize("notify.delete_profile_error", {
            "{error}": "HA not connected",
          }),
          "error",
        );
      ...
      const confirmed =
        typeof window !== "undefined"
          ? window.confirm(
              localize("prompt.delete_profile_confirm", {
                "{profile}": current,
              }),
            )
          : true;
      ...
      try {
        const options = Array.isArray(this.card.profileOptions)
          ? [...this.card.profileOptions]
          : [];
      ...
      this.showNotification(
        localize("notify.delete_profile_success", { "{profile}": current }),
        "success",
      );
    } catch (err) {
  ...
  handleHelp() {
    ...
    const preset =
      this.card.config?.preset_type || this.card.config?.preset || "thermostat";
    const prefix = getEffectivePrefix(this.card.config);
    const targetEntity = this.card.config?.target_entity || "Not configured";
    const profileEntity =
      this.card.config?.profiles_select_entity || "Not configured";
    ...
    const availableProfiles =
      (this.card.profileOptions || []).join(", ") || "None found";
...
1. Profiles: /config/cronostar/profiles/${prefixBase}_data.json`;
    ...
    const sections = [
      ...
      {
        title: localize("help.mouse_usage_title", "Mouse Usage"),
        text: mouseManual,
      },
      {
        title: localize("help.keyboard_usage_title", "Keyboard Usage"),
        text: keyboardManual,
      },
    ];
    ...
    techTitle.textContent = localize(
      "help.technical_details_title",
      "Technical Details",
    );
    ...
    copyBtn.textContent = localize(
      "help.copy_technical_details_button",
  ...
  async toggleEnabled(e) {
    try {
      ...
      Logger.log(
        "UI",
        `[CronoStar] toggleEnabled: calling ${domain}.${service} on ${entityId}`,
      );
      await this.card.hass.callService(domain, service, {
        entity_id: entityId,
      });
  ...
  handleEditCard() {
    this.card.dispatchEvent(
      new CustomEvent("hass-edit-card", {
        bubbles: true,
        composed: true,
      }),
    );
