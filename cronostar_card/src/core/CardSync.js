...
export class CardSync {
  ...
  scheduleAutomationOverlaySuppression(ms = TIMEOUTS.automationSuppression) {
    ...
    Logger.log(
      "SYNC",
      `[CronoStar] Suppressing automation overlay until ${new Date(this.card.overlaySuppressionUntil).toLocaleTimeString()} (cooldown=${ms || 7000}ms)`,
    );
  ...
  getTargetEntityAppliedValue(hass) {
    try {
      ...
      if (domain === "climate") {
        ...
        if (
          attrs.target_temperature !== undefined &&
          attrs.target_temperature !== null
        ) {
  ...
  updateAutomationSync(hass) {
    ...
    const tolerance = isSwitchPreset
      ? 0.5
      : Math.max(0.05, (this.card.config?.step_value || 0.5) / 2);
    ...
    if (!mismatchNow) {
      ...
      if (!this.card.mismatchSince) {
        ...
        this.card.awaitingAutomation =
          persisted >= TIMEOUTS.mismatchPersistenceMs;

        if (this.card.awaitingAutomation) {
          const hourIdx = this.card.stateManager?.getCurrentIndex() || 0;
          const hourLabel =
            this.card.stateManager?.getPointLabel(hourIdx) || "Now";
          const details =
          ...
          if (!wasAwaiting) {
            Logger.log(
              "SYNC",
              `[CronoStar] Awaiting automation started (persisted ${persisted}ms): scheduled=${scheduled}, applied=${applied}`,
            );
          }
        } else {
          this.card.outOfSyncDetails = "";
