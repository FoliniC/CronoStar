...
export class Step5Summary {
  ...
  render() {
    ...
    try {
      ...
      console.log('[Step5Summary] Entities:', { currentEntity, selectEntity, enabledEntity });
      ...
      console.log('[Step5Summary] Config complete:', configComplete, 'Missing:', missingFields);
      ...
      return html`
        <div class="step-content">
          ...
          <div class="field-group" style="border: 1px solid ${configComplete ? '#22c55e' : '#ef4444'}; background: ${configComplete ? 'rgba(34, 197, 94, 0.05)' : 'rgba(239, 68, 68, 0.05)'}; padding: 20px; border-radius: 12px; margin-bottom: 24px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
              <div style="display: flex; align-items: center; gap: 12px;">
                <img src="/cronostar_card/cronostar-logo.png" style="width: 24px; height: auto;" alt="CronoStar">
                <div style="font-weight: 700; font-size: 1.1rem; color: #ffffff;">
                   ${this.editor._config.title || 'CronoStar Controller'}
                </div>
              </div>
              ${configComplete 
                ? html`<span style="background: #22c55e; color: #ffffff; padding: 4px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 800;">VALID</span>`
            ...
            <div style="font-size: 0.85rem; color: #94a3b8; display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 20px;">
               <div style="background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 6px;">
                  <strong>Entity:</strong> <span style="font-family: monospace; color: #ffffff;">${this.editor._config.target_entity || 'N/A'}</span>
               </div>
               <div style="background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 6px;">
                  <strong>Prefix:</strong> <span style="font-family: monospace; color: #ffffff;">${effectivePrefix}</span>
               </div>
            </div>
          ...
          <details style="margin-bottom: 24px; cursor: pointer;">
            <summary style="color: #94a3b8; font-size: 0.9rem; margin-bottom: 8px;">${isIt ? 'Mostra YAML configurazione' : 'Show YAML configuration'}</summary>
            <div style="background: #0f172a; padding: 16px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1);">
              <pre style="margin: 0; color: #38bdf8; font-size: 0.85rem; font-family: 'Fira Code', monospace; white-space: pre-wrap;">${cardYaml}</pre>
            </div>
          </details>
      ...
      return html`<div style="color: red; padding: 20px;">Error rendering Step 5: ${e.message}</div>`;
    }
  }
}