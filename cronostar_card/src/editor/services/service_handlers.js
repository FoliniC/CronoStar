...
export async function handleInitializeData(hass, config, language) {
  ...
  try {
    ...
    if (resp?.schedule && Array.isArray(resp.schedule)) {
      ...
      Logger.log(
        "INIT",
        `Loaded existing profile '${profileName}' for analysis/correction`,
      );
    } else {
      isNew = true;
    }
    ...
    Logger.log(
      "INIT",
      `Profile '${profileName}' not found, initializing fresh default`,
    );
  }
  ...
  return {
    ...
    message: isNew
      ...
      : "✓ Existing profile analyzed and corrected",
  };
}
