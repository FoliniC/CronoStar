...
export class KeyboardHandler {
  ...
  handleKeydown(e) {
    Logger.log(
      "KEYBOARD",
      `[CronoStar] Keydown: ${e.key}, enabled: ${this.enabled}`,
    );
    ...
    const isCtrlOrMeta =
      this.ctrlDown || this.metaDown || e.ctrlKey || e.metaKey;
    ...
    if (indices.length === 0) {
      Logger.log(
        "KEYBOARD",
        "[CronoStar] No points selected, ignoring arrow keys",
      );
      return;
    }
  ...
  handleInsertPoint() {
    ...
    const midValue =
      (scheduleData[anchorIndex].value + scheduleData[anchorIndex + 1].value) /
      2;
  ...
  handleArrowLeftRight(e, indices) {
    ...
    const settings = {
      def: {
        h:
          config.kb_def_h !== undefined
            ? config.kb_def_h
            : kbGlobal.def.horizontal,
        v:
          config.kb_def_v !== undefined
            ? config.kb_def_v
            : kbGlobal.def.vertical,
      },
      ctrl: {
        h:
          config.kb_ctrl_h !== undefined
            ? config.kb_ctrl_h
            : kbGlobal.ctrl.horizontal,
        v:
          config.kb_ctrl_v !== undefined
            ? config.kb_ctrl_v
            : kbGlobal.ctrl.vertical,
      },
      shift: {
        h:
          config.kb_shift_h !== undefined
            ? config.kb_shift_h
            : kbGlobal.shift.horizontal,
        v:
          config.kb_shift_v !== undefined
            ? config.kb_shift_v
            : kbGlobal.shift.vertical,
      },
      alt: {
        h:
          config.kb_alt_h !== undefined
            ? config.kb_alt_h
            : kbGlobal.alt.horizontal,
        v:
          config.kb_alt_v !== undefined
            ? config.kb_alt_v
            : kbGlobal.alt.vertical,
      },
    };
    ...
    if (config.is_switch_preset) {
      ...
      indices.forEach((idx) => {
        ...
        data.forEach((pt, i) => {
          ...
          if (Math.abs(pt.x - curX) <= 1.5) {
            // tolerant check like chart_manager
            partners.push(i);
          }
        });
      });
      ...
      Logger.log(
        "SWITCH",
        `[Keyboard] Expanded selection for drag: ${JSON.stringify(indices)} -> ${JSON.stringify(indicesToMove)}`,
      );
    }
    ...
    const firstSortedSelectedPos = allByTime.findIndex((item) =>
      selectedSet.has(item.idx),
    );
    ...
    if (
      lastSortedSelectedPos !== -1 &&
      lastSortedSelectedPos < allByTime.length - 1
    ) {
    ...
    indicesToMove.forEach((i) => {
      ...
      Logger.log(
        "SWITCH",
        `[KeyboardMove] Point ${i}: ${minutesToTime(oldX)} -> ${minutesToTime(p.x)}`,
      );
    });
  ...
  handleArrowUpDown(e, indices) {
    ...
    const settings = {
      def:
        config.kb_def_v !== undefined ? config.kb_def_v : kbGlobal.def.vertical,
      ctrl:
        config.kb_ctrl_v !== undefined
          ? config.kb_ctrl_v
          : kbGlobal.ctrl.vertical,
      shift:
        config.kb_shift_v !== undefined
          ...
          : kbGlobal.shift.vertical,
      alt:
        config.kb_alt_v !== undefined ? config.kb_alt_v : kbGlobal.alt.vertical,
    };
    ...
    const upperClamp =
      this.card.config.allow_max_value && !this.card.config.is_switch_preset
    ...
    const selectedTimes = indices
      .map((i) => (data[i]?.x !== undefined ? minutesToTime(data[i].x) : null))
      .filter((t) => t !== null);
  ...
  attachListeners(element) {
    if (!element) {
      Logger.error(
        "KEYBOARD",
        "[CronoStar] Cannot attach listeners: element is null",
      );
      return;
    }
