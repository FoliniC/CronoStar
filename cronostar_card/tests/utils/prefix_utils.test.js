...
import {
  ...
  getEffectivePrefix,
  getAliasWithPrefix,
} from "../../src/utils/prefix_utils.js";
...
describe("prefix_utils", () => {
  ...
  describe("getEffectivePrefix", () => {
    it("should return global_prefix if provided", () => {
      expect(getEffectivePrefix({ global_prefix: "my_prefix_" })).toBe(
        "my_prefix_",
      );
    });
  ...
  describe("getAliasWithPrefix", () => {
    it("should build a localized alias", () => {
      expect(getAliasWithPrefix("living_room_", "en")).toBe(
        "CronoStar - apply living room",
      );
      expect(getAliasWithPrefix("living_room_", "it")).toBe(
        "CronoStar - applica living room",
      );
    });
  });
});
