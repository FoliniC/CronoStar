...
describe("main.js", () => {
  ...
  describe("global custom element registration", () => {
    it("should register cronostar-card and editor elements globally", async () => {
      ...
      expect(customElementsDefineSpy).toHaveBeenCalledWith(
        "custom:cronostar-card",
        mocks.CronoStarCard,
      );
      expect(customElementsDefineSpy).toHaveBeenCalledWith(
        "cronostar-card-editor",
        mocks.CronoStarEditor,
      );
      expect(customElementsDefineSpy).toHaveBeenCalledWith(
        "custom:cronostar-card-editor",
        mocks.CronoStarEditor,
      );

      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('✅ "cronostar-card" registrato in global'),
      );
    });
    ...
    it("should warn if registry is null during registration", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'cronostar-card" risulta già definito in global',
        ),
      );
    });
    ...
    it("should log if element is already defined", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        'CRONOSTAR: "cronostar-card" già registrato in global',
      );
      // Should NOT call define for cronostar-card
      expect(customElementsDefineSpy).not.toHaveBeenCalledWith(
        "cronostar-card",
        mocks.CronoStarCard,
      );
    });
    ...
    it("should log an error if define fails for an unknown reason", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'CRONOSTAR: ❌ Errore registrazione "cronostar-card" in global:',
        ),
        expect.any(Error),
      );
    });
  ...
  describe("ScopedRegistryHost patching", () => {
    ...
    it("should patch connectedCallback if ScopedRegistryHost is defined", async () => {
      ...
      window.ScopedRegistryHost.prototype.connectedCallback =
      ...
      expect(window.ScopedRegistryHost.prototype.connectedCallback).not.toBe(
        originalConnectedCallback,
      );
      ...
      expect(mockRegistry.define).toHaveBeenCalledWith(
        "cronostar-card",
        mocks.CronoStarCard,
      );
      expect(mockRegistry.define).toHaveBeenCalledWith(
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "CRONOSTAR: Scoped host connesso: scoped(<mock-host>)",
        ),
      );
    });
  });
  ...
  describe("window.customCards registration", () => {
    ...
    it("should add cronostar-card to window.customCards if not existing", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        "CRONOSTAR: ✅ Aggiunto a window.customCards",
      );
    });
    ...
    it("should update cronostar-card in window.customCards if already existing", async () => {
      ...
      expect(window.customCards[0]).toEqual(cardMetadata);
      expect(consoleLogSpy).toHaveBeenCalledWith(
        "CRONOSTAR: ✅ Aggiornata registrazione in window.customCards",
      );
    });
  });
  ...
  it("should log initialization complete message", async () => {
    await import("../src/main.js");
    expect(consoleLogSpy).toHaveBeenCalledWith(
      "CRONOSTAR: Inizializzazione main.js completata ✅",
    );
  });
});
