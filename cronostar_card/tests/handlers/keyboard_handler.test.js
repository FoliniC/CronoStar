...
describe("KeyboardHandler", () => {
  ...
  beforeEach(() => {
    card = {
      ...
      stateManager: {
        ...
        setData: vi.fn(),
        scheduleData: Array(10)
          .fill({ time: "00:00", value: 20 })
          .map((p, i) => ({ ...p, time: `0${i}:00` })),
      },
      selectionManager: {
        selectAll: vi.fn(),
  ...
  it("should handle modifier keys", () => {
    ...
    const up = (key) =>
      container.dispatchEvent(new KeyboardEvent("keyup", { key }));

    down("Control");
    expect(handler.ctrlDown).toBe(true);
    up("Control");
    expect(handler.ctrlDown).toBe(false);

    down("Meta");
    expect(handler.metaDown).toBe(true);
    up("Meta");
    expect(handler.metaDown).toBe(false);

    down("Shift");
    expect(handler.shiftDown).toBe(true);
    up("Shift");
    expect(handler.shiftDown).toBe(false);

    down("Alt");
    expect(handler.altDown).toBe(true);
    up("Alt");
    expect(handler.altDown).toBe(false);
  });
