...
@pytest.mark.anyio
async def test_load_all_profiles_success(tmp_path):
    """Test caricamento di tutti i profili con validazione.

    FileChecker è importato localmente dentro load_all_profiles, quindi
    va patchato nel modulo originale.
    """
    hass = _make_hass(tmp_path)
    storage = _make_storage(hass, tmp_path)
    ...
    mock_checker._validate_profile_file = AsyncMock(return_value={"valid": True, "errors": []})

    with patch(
        "custom_components.cronostar.deep_checks.file_checker.FileChecker",
        return_value=mock_checker,
    ):
        result = await storage.load_all_profiles()

    assert filename in result
    assert "validation_results" in result[filename]


@pytest.mark.anyio
async def test_load_all_profiles_empty_dir(tmp_path):
    """Test caricamento profili con directory vuota."""
    hass = _make_hass(tmp_path)
    storage = _make_storage(hass, tmp_path)
...
@pytest.mark.anyio
async def test_load_all_profiles_invalid_container(tmp_path):
    """Test caricamento con container non valido."""
    hass = _make_hass(tmp_path)
    storage = _make_storage(hass, tmp_path)

    filepath = storage.profiles_dir / "cronostar_bad_.json"
    filepath.parent.mkdir(parents=True, exist_ok=True)
    filepath.write_text("{ invalid }", encoding="utf-8")

    mock_checker = MagicMock()
    mock_checker._validate_profile_file = AsyncMock(return_value={"valid": False, "errors": ["parse error"]})
    ...
    assert "cronostar_bad_.json" in result
    assert result["cronostar_bad_.json"]["validation_results"]["valid"] is False
