...
class TestRegisterLovelaceDashboard:
    ...
    def test_lovelace_absent_from_hass_data(self, mock_hass):
        """Branch: 'lovelace' not in hass.data → logs error and returns early."""
        mock_hass.data = {}  # no 'lovelace' key
        run(_register_lovelace_dashboard(mock_hass, "/tmp/fake.yaml"))
        # Should not raise; the early-return path must be hit

    def test_dashboards_attr_none_and_not_dict(self, mock_hass):
    ...
    def test_lovelace_data_is_dict_with_dashboards(self, mock_hass):
        ...
        with patch(
            ...
            run(_register_lovelace_dashboard(mock_hass, "/tmp/my_dash.yaml"))

        assert PANEL_URL_PATH in dashboards

    def test_lovelace_data_is_dict_without_dashboards_key(self, mock_hass):
        """Branch: lovelace_data is a dict but has no 'dashboards' key → logs error."""
        mock_hass.data = {"lovelace": {"other_key": "value"}}
        run(_register_lovelace_dashboard(mock_hass, "/tmp/fake.yaml"))
        # Must reach the "dashboards is None" error branch without raising

    def test_existing_entry_is_removed_before_re_registration(self, mock_hass):
    ...
    def test_lovelace_data_object_with_dashboards_attribute(self, mock_hass):
    ...
    def test_register_raises_is_caught(self, mock_hass):
...
class TestWriteDashboardYaml:
    ...
    def test_with_real_controller_entry(self, mock_hass, tmp_path):
    ...
    def test_file_write_error_is_caught(self, mock_hass, tmp_path):
        ...
        mock_hass.async_add_executor_job = _failing_exec

        # Must not propagate
        run(write_dashboard_yaml(mock_hass, "cronostar_test.yaml"))

    def test_entry_with_none_optional_fields_excluded(self, mock_hass, tmp_path):
...
class TestPurgeOldStorageFiles:
    ...
    def test_purge_removes_matching_files(self, mock_hass, tmp_path):
        ...
        with (
            ...
            run(setup_dashboard(mock_hass))

        assert not stale.exists()

    def test_purge_os_remove_error_is_caught(self, mock_hass, tmp_path):
...
class TestSetupDashboard:
    ...
    def test_async_remove_panel_exception_is_swallowed(self, mock_hass, tmp_path):
        ...
        with (
            ...
            run(setup_dashboard(mock_hass))  # must not raise

    def test_critical_failure_is_caught(self, mock_hass, tmp_path):
    ...
    def test_full_happy_path(self, mock_hass, tmp_path):
