...
def test_delete_profile_not_found(hass):
    ...
    success = run(manager.delete_profile("P2", "thermostat", "prefix"))
    assert success is False

def test_delete_profile_container_not_found(hass):
...
def test_list_profiles_filters(hass):
    ...
    with patch("pathlib.Path.glob", return_value=[p1, p2]):
        ...
        # Filter by preset
        files = run(manager.list_profiles(preset_type="thermostat"))
        assert len(files) == 1
        ...
        files = run(manager.list_profiles(prefix="p2_"))
        assert len(files) == 1
        assert files[0] == "cronostar_t2.json"

def test_get_profile_list(hass):
    ...
    profiles = run(manager.get_profile_list("thermostat", "prefix"))
    assert "P1" in profiles
    assert "P2" in profiles

def test_get_cached_containers(hass):
    ...
    res = run(manager.get_cached_containers(preset_type="thermostat"))
    assert len(res) == 1
    assert res[0][0] == "f1.json"

def test_write_json_error(hass):
    """Test error handling in _write_json."""
    manager = StorageManager(hass, hass.config.path("cronostar/profiles"))
    # In conftest.py, hass is a MagicMock, and async_add_executor_job is typically patched
    hass.async_add_executor_job = AsyncMock(side_effect=Exception("Write error"))
    
    with pytest.raises(Exception, match="Write error"):
        run(manager._write_json(Path("test.json"), {}))

def test_load_container_json_error(hass):
    """Test error handling in _load_container with invalid JSON."""
    manager = StorageManager(hass, hass.config.path("cronostar/profiles"))
    with patch("pathlib.Path.exists", return_value=True):
        hass.async_add_executor_job.return_value = "invalid json"
        res = run(manager._load_container(Path("test.json")))
        assert res == {}
