...
def test_profile_service_load_profile_error(hass, profile_service):
...
def test_profile_service_load_profile_exception(hass, profile_service):
    """Hit lines 199-201 in load_profile (exception)."""
    call = MagicMock()
    call.data = {"profile_name": "Error"}
    with patch.object(profile_service, 'get_profile_data', side_effect=Exception("Fail")):
        res = run(profile_service.load_profile(call))
        assert "error" in res

def test_get_profile_data_loop_branches(hass, profile_service, mock_storage_manager):
    ...
    run(profile_service.get_profile_data("P1", "thermostat"))

def test_profile_service_validate_schedule_invalid_types(profile_service):
...
def test_setup_services_list_all_error(hass, mock_storage_manager):
