...
def test_coordinator_logging_enabled(hass):
...
def test_coordinator_storage_fallback(hass):
...
def test_async_update_data_target_missing(hass, mock_coordinator):
    """Test _async_update_data when target entity is missing."""
    # Ensure it's missing (should be by default)
    mock_coordinator.target_entity = "climate.missing"
    
    result = run(mock_coordinator._async_update_data())
    assert result["current_value"] == mock_coordinator.current_value

def test_async_initialize_no_profiles(hass, mock_coordinator, mock_storage_manager):
    """Test async_initialize when no profiles are found."""
    mock_storage_manager.list_profiles.return_value = []
    run(mock_coordinator.async_initialize())
    assert mock_coordinator.available_profiles == ["Default"]

def test_async_initialize_exception(hass, mock_coordinator, mock_storage_manager):
    """Test async_initialize with an exception."""
    mock_storage_manager.list_profiles.side_effect = Exception("Storage error")
    # Should not raise
    run(mock_coordinator.async_initialize())

def test_async_refresh_profiles_no_files(hass, mock_coordinator, mock_storage_manager):
    """Test async_refresh_profiles when no files exist."""
    mock_storage_manager.list_profiles.return_value = []
    run(mock_coordinator.async_refresh_profiles())
    # Should maintain current state or default
    assert mock_coordinator.available_profiles == ["Default"]

def test_async_refresh_profiles_exception(hass, mock_coordinator, mock_storage_manager):
    """Test async_refresh_profiles with an exception."""
    mock_storage_manager.list_profiles.side_effect = Exception("Storage error")
    # Should not raise
    run(mock_coordinator.async_refresh_profiles())

def test_apply_schedule_disabled(hass, mock_coordinator):
    """Test apply_schedule when coordinator is disabled."""
    mock_coordinator.is_enabled = False
    run(mock_coordinator.apply_schedule())
    # Should return early before doing anything

def test_apply_schedule_empty_container(hass, mock_coordinator, mock_storage_manager):
    """Test apply_schedule with empty container."""
    mock_storage_manager.load_profile_cached.return_value = {}
    run(mock_coordinator.apply_schedule())

def test_interpolate_schedule_invalid_points(mock_coordinator):
...
def test_interpolate_schedule_single_point(mock_coordinator):
