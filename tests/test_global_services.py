...
async def test_setup_services(mock_hass):
    ...
    await setup_services(mock_hass, storage_manager)
...
def get_handler(mock_hass, service_name):
...
async def test_save_profile_service(mock_hass):
    storage_manager = MagicMock()
    await setup_services(mock_hass, storage_manager)
    handler = get_handler(mock_hass, "save_profile")
    
    profile_service = mock_hass.data["cronostar"]["profile_service"]
...
async def test_load_profile_service(mock_hass):
    storage_manager = MagicMock()
    await setup_services(mock_hass, storage_manager)
    handler = get_handler(mock_hass, "load_profile")
...
async def test_add_profile_service(mock_hass):
    storage_manager = MagicMock()
    await setup_services(mock_hass, storage_manager)
    handler = get_handler(mock_hass, "add_profile")
...
async def test_delete_profile_service(mock_hass):
    storage_manager = MagicMock()
    await setup_services(mock_hass, storage_manager)
    handler = get_handler(mock_hass, "delete_profile")
...
async def test_apply_now_service(mock_hass):
    """Test apply_now service handler."""
    storage_manager = MagicMock()
    await setup_services(mock_hass, storage_manager)
    handler = get_handler(mock_hass, "apply_now")
    ...
    mock_hass.services.async_call.assert_called_with(
        "climate", "set_temperature", {"entity_id": "climate.test", "temperature": 20.0}, blocking=False
    )
