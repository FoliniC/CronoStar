...
def test_coordinator_unsupported_domain(hass):
...
def test_coordinator_update_exception(hass):
...
def test_coordinator_interpolate_edge_cases(hass):
...
def test_profile_service_ensure_controller_no_prefix(hass):
    """Test _ensure_controller_exists with empty prefix."""
    ps = ProfileService(hass, MagicMock(), MagicMock())
    run(ps._ensure_controller_exists("", "thermostat", {}))
    assert not hass.config_entries.flow.async_init.called

def test_profile_service_update_selectors_load_fail(hass):
    ...
    run(ps.async_update_profile_selectors())
    # Should continue

def test_storage_get_cached_containers_filters(hass):
    ...
    # Mismatched preset
    assert len(run(manager.get_cached_containers(preset_type="fan"))) == 0
    # Mismatched prefix
    assert len(run(manager.get_cached_containers(global_prefix="p2"))) == 0
    # Match
    assert len(run(manager.get_cached_containers(preset_type="thermostat", global_prefix="p1"))) == 1
