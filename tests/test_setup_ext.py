...
def test_validate_environment_all_checks(hass):
    ...
    with patch("pathlib.Path.exists", return_value=True), \
        ...
        success = run(validate_environment(hass))
...
def test_setup_integration_preload_failure(hass):
    ...
    with patch("custom_components.cronostar.setup.validate_environment", return_value=True), \
        ...
        success = run(async_setup_integration(hass, config))
        assert success is True

def test_setup_static_resources_http_missing(hass):
