...
def _make_coordinator(hass, mock_entry, **entry_data_overrides):
    """Build a CronoStarCoordinator from a mock entry."""
    # ✅ real dict — MagicMock.update() has no effect
    base_data = {
    ...
    base_data.update(entry_data_overrides)
    mock_entry.data = base_data   # ← overwrite the MagicMock
    mock_entry.options = {}
...
class TestAsyncUpdateData:
    ...
    def test_target_entity_missing_returns_last_state(self, hass, mock_entry):
        """Branch: hass.states.get returns None → early return with cached state."""
        coord, _ = _make_coordinator(hass, mock_entry)
        coord.target_entity = "climate.missing" # Ensure missing
    ...
    def test_target_entity_missing_with_logging(self, hass, mock_entry):
        """Same early-return but with logging_enabled=True (debug log path)."""
        coord, _ = _make_coordinator(hass, mock_entry)
        coord.logging_enabled = True
        coord.target_entity = "climate.missing"

        result = run(coord._async_update_data())
        assert "selected_profile" in result

    def test_target_entity_present_calls_apply_schedule(self, hass, mock_entry):
        """Branch: entity present → apply_schedule called."""
        coord, _ = _make_coordinator(hass, mock_entry)
        # Mock states.get to return a valid state
...
class TestApplySchedule:
    ...
    def test_disabled_controller_skips(self, hass, mock_entry):
    ...
    def test_disabled_with_logging(self, hass, mock_entry):
        coord, _ = _make_coordinator(hass, mock_entry)
        coord.is_enabled = False
        coord.logging_enabled = True
        coord._update_target_entity = AsyncMock()
    ...
    def test_target_state_unavailable_skips(self, hass, mock_entry):
        """Branch: target entity state is STATE_UNAVAILABLE → return early."""
        coord, _ = _make_coordinator(hass, mock_entry)
        hass.states.async_set("climate.test_entity", "unavailable")
        coord._update_target_entity = AsyncMock()

        run(coord.apply_schedule())
        coord._update_target_entity.assert_not_called()

    def test_target_state_unknown_skips(self, hass, mock_entry):
        """Branch: target entity state is STATE_UNKNOWN → return early."""
        coord, _ = _make_coordinator(hass, mock_entry)
        hass.states.async_set("climate.test_entity", "unknown")
        coord._update_target_entity = AsyncMock()

        run(coord.apply_schedule())
        coord._update_target_entity.assert_not_called()

    def test_target_state_none_skips(self, hass, mock_entry):
    ...
    def test_unavailable_with_logging_enabled(self, hass, mock_entry):
    ...
    def test_no_files_found_no_update(self, hass, mock_entry):
        """Branch: list_profiles returns [] → schedule stays empty → no update."""
        coord, sm = _make_coordinator(hass, mock_entry)
        hass.states.async_set("climate.test_entity", "heat")
        sm.list_profiles = AsyncMock(return_value=[])
        coord._update_target_entity = AsyncMock()
    ...
    def test_profile_not_in_container(self, hass, mock_entry):
    ...
    def test_profiles_sync_from_filesystem(self, hass, mock_entry):
    ...
    def test_storage_error_in_apply_schedule(self, hass, mock_entry):
...
class TestUpdateTargetEntity:
    ...
    def _coord_for(self, hass, mock_entry, domain="climate"):
        # ✅ pass target_entity as override, don't try to write to MagicMock
        coord, _ = _make_coordinator(
            hass, mock_entry,
            target_entity=f"{domain}.test_entity"
        )
        return coord

    def test_climate_domain(self, hass, mock_entry):
        coord = self._coord_for(hass, mock_entry, "climate")
        run(coord._update_target_entity(21.5))
        hass.services.async_call.assert_called_once()
        args = hass.services.async_call.call_args
        assert args[0][0] == "climate"

    def test_switch_on(self, hass, mock_entry):
        coord = self._coord_for(hass, mock_entry, "switch")
        run(coord._update_target_entity(1.0))
        args = hass.services.async_call.call_args
        assert args[0][1] == "turn_on"

    def test_switch_off(self, hass, mock_entry):
        coord = self._coord_for(hass, mock_entry, "switch")
        run(coord._update_target_entity(0.0))
        args = hass.services.async_call.call_args
        assert args[0][1] == "turn_off"

    def test_light_on(self, hass, mock_entry):
        coord = self._coord_for(hass, mock_entry, "light")
        run(coord._update_target_entity(1.0))
        args = hass.services.async_call.call_args
        assert args[0][0] == "light"

    def test_fan_off(self, hass, mock_entry):
        coord = self._coord_for(hass, mock_entry, "fan")
        run(coord._update_target_entity(0.0))
        args = hass.services.async_call.call_args
        assert args[0][1] == "turn_off"

    def test_input_number_domain(self, hass, mock_entry):
        coord = self._coord_for(hass, mock_entry, "input_number")
        run(coord._update_target_entity(50.0))
        args = hass.services.async_call.call_args
        assert args[0][0] == "input_number"

    def test_cover_domain(self, hass, mock_entry):
        coord = self._coord_for(hass, mock_entry, "cover")
    ...
    def test_unsupported_domain_warning(self, hass, mock_entry):
        """Branch: unknown domain → no service call + optional warning logged."""
        coord = self._coord_for(hass, mock_entry, "sensor")
        run(coord._update_target_entity(10.0))
        hass.services.async_call.assert_not_called()

    def test_unsupported_domain_with_logging(self, hass, mock_entry):
    ...
    def test_service_call_exception_caught(self, hass, mock_entry):
        """Branch: async_call raises → error logged, no re-raise."""
        coord = self._coord_for(hass, mock_entry, "climate")
        hass.services.async_call = AsyncMock(side_effect=Exception("timeout"))
        run(coord._update_target_entity(20.0))  # must not raise

    def test_service_call_exception_with_logging(self, hass, mock_entry):
...
class TestAsyncInitialize:
    ...
    def test_no_files_found(self, hass, mock_entry):
    ...
    def test_with_profiles_last_active_restored(self, hass, mock_entry):
    ...
    def test_selected_not_in_available_falls_to_default(self, hass, mock_entry):
    ...
    def test_selected_not_in_available_falls_to_first(self, hass, mock_entry):
    ...
    def test_initialize_exception_caught(self, hass, mock_entry):
    ...
    def test_no_files_with_logging_disabled(self, hass, mock_entry):
    ...
    def test_profiles_loaded_with_logging(self, hass, mock_entry):
...
class TestAsyncRefreshProfiles:
    ...
    def test_selected_not_in_updated_profiles(self, hass, mock_entry):
    ...
    def test_selected_removed_falls_to_first(self, hass, mock_entry):
    ...
    def test_refresh_exception_caught(self, hass, mock_entry):
    ...
    def test_refresh_with_logging(self, hass, mock_entry):
...
class TestSetProfileAndEnabled:
    ...
    def test_set_profile_not_in_available(self, hass, mock_entry):
    ...
    def test_set_profile_valid(self, hass, mock_entry):
        coord, sm = _make_coordinator(hass, mock_entry)
    ...
    def test_set_profile_with_logging(self, hass, mock_entry):
        coord, sm = _make_coordinator(hass, mock_entry)
        coord.available_profiles = ["Default", "Summer"]
        coord.logging_enabled = True
        coord.async_refresh = AsyncMock()
    ...
    def test_set_profile_not_found_with_logging(self, hass, mock_entry):
    ...
    def test_set_enabled(self, hass, mock_entry):
        coord, _ = _make_coordinator(hass, mock_entry)
        coord.async_refresh = AsyncMock()
        coord.logging_enabled = True
...
class TestInterpolateSchedule:
    ...
    def _coord(self, hass, mock_entry):
        coord, _ = _make_coordinator(hass, mock_entry)
        return coord

    def test_empty_schedule_returns_none(self, hass, mock_entry):
        coord = self._coord(hass, mock_entry)
        assert coord._interpolate_schedule([]) is None

    def test_invalid_schedule_point_skipped(self, hass, mock_entry):
        coord = self._coord(hass, mock_entry)
        coord.logging_enabled = True
        result = coord._interpolate_schedule([{"time": "bad", "value": 20}])
        assert result is None

    def test_single_point_returns_that_value(self, hass, mock_entry):
        coord = self._coord(hass, mock_entry)
    ...
    def test_exact_match(self, hass, mock_entry):
        coord = self._coord(hass, mock_entry)
        now = datetime.now()
    ...
    def test_generic_switch_uses_step_not_interpolated(self, hass, mock_entry):
    ...
    def test_midnight_wraparound(self, hass, mock_entry):
    ...
    def test_t1_equals_t2_returns_v1(self, hass, mock_entry):
    ...
    def test_all_points_invalid_returns_none(self, hass, mock_entry):
    ...
    def test_missing_time_key_skipped(self, hass, mock_entry):
        coord = self._coord(hass, mock_entry)
...
class TestGetNextChange:
    ...
    def test_all_same_value_no_change(self, hass, mock_entry):
    ...
    def test_forward_change_found(self, hass, mock_entry):
    ...
    def test_wrap_around_change(self, hass, mock_entry):
    ...
    def test_invalid_time_point_skipped(self, hass, mock_entry):
        coord = self._coord(hass, mock_entry)
        result = coord._get_next_change(
    ...
    def test_exception_returns_none(self, hass, mock_entry):
...
class TestMinutesToTime:
    ...
    def test_basic(self, hass, mock_entry):
        coord, _ = _make_coordinator(hass, mock_entry)
        assert coord._minutes_to_time(90) == "01:30"

    def test_overflow_wraps(self, hass, mock_entry):
        coord, _ = _make_coordinator(hass, mock_entry)
        # 1440 + 60 = 1500 → 01:00
...
class TestCoordinatorInit:
    ...
    def test_storage_fallback_when_not_in_hass_data(self, hass, mock_entry):
    ...
    def test_prefix_built_from_name_when_global_prefix_absent(self, hass, mock_entry):
        ...
        hass.data = {DOMAIN: {"storage_manager": MagicMock(), "global_config": {}}}
        coord = CronoStarCoordinator(hass, mock_entry)
        assert "my_controller" in coord.prefix

    def test_logging_enabled_from_hass_data(self, hass, mock_entry):
    ...
    def test_preset_fallback_key(self, hass, mock_entry):
...
class TestLoggingBranches:
    ...
    def test_update_data_debug_log_when_entity_present(self, hass, mock_entry):
    ...
    def test_initialize_logs_restored_profile(self, hass, mock_entry):
    ...
    def test_apply_schedule_logs_profile_not_found_warning(self, hass, mock_entry):
    ...
    def test_apply_schedule_logs_no_value_interpolated(self, hass, mock_entry):
...
class TestInterpolateScheduleWithMockedTime:
    ...
    def test_prev_point_from_wraparound_line_400(self, hass, mock_entry):
    ...
    def test_midnight_crossing_adds_1440_line_421_423(self, hass, mock_entry):
    ...
    def test_t2_equals_t1_returns_v1_line_425(self, hass, mock_entry):
