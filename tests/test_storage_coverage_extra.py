...
@pytest.mark.anyio
async def test_save_profile_cache_mtime_error(hass):
    ...
    with patch("os.path.getmtime", side_effect=OSError("File missing during mtime check")):
        await manager.save_profile("P", "thermostat", {}, {}, "prefix")
...
@pytest.mark.anyio
async def test_load_profile_cached_mtime_error(hass):
...
@pytest.mark.anyio
async def test_load_profile_cached_update_mtime_error(hass):
...
@pytest.mark.anyio
async def test_delete_profile_cache_mtime_error(hass):
    ...
    with patch("os.path.getmtime", side_effect=OSError("Error")):
        await manager.delete_profile("P1", "thermostat", "p")
    
    assert manager._cache_mtimes[filename] == 0

@pytest.mark.anyio
async def test_list_profiles_normalize_error(hass):
    ...
    with patch("pathlib.Path.glob", return_value=[MagicMock(name="cronostar_p_data.json")]):
        ...
        with patch("pathlib.Path.glob", return_value=[filepath]):
            ...
            with patch("custom_components.cronostar.utils.prefix_normalizer.normalize_preset_type", side_effect=Exception("Import error")):
                res = await manager.list_profiles(preset_type="t")
                assert len(res) == 1

@pytest.mark.anyio
async def test_update_active_profile_mtime_error(hass):
    ...
    with patch("os.path.getmtime", side_effect=OSError("Error")):
        await manager.update_active_profile("t", "p", "P1")
