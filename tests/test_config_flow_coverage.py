...
@pytest.mark.anyio
async def test_async_step_create_controller_already_configured(hass):
...
@pytest.mark.anyio
async def test_async_step_reconfigure_component(hass):
    ...
    # Submit
    flow.async_update_reload_and_abort = MagicMock(return_value="success")
    result = await flow.async_step_reconfigure(user_input={CONF_LOGGING_ENABLED: True})
    assert result == "success"
    flow.async_update_reload_and_abort.assert_called_once()

@pytest.mark.anyio
async def test_async_step_reconfigure_controller(hass):
    ...
    # Submit
    flow.async_update_reload_and_abort = MagicMock(return_value="success")
    result = await flow.async_step_reconfigure(user_input={CONF_NAME: "New"})
    assert result == "success"

@pytest.mark.anyio
async def test_async_step_user_not_installed(hass):
...
@pytest.mark.anyio
async def test_async_step_install_component_submit(hass):
...
@pytest.mark.anyio
async def test_async_step_dashboard_with_dashboards(hass):
...
@pytest.mark.anyio
async def test_async_add_card_to_dashboard_logic(hass):
...
@pytest.mark.anyio
async def test_options_flow_init_global(hass):
