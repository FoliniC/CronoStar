...
from custom_components.cronostar.utils.error_handler import log_operation
from custom_components.cronostar.utils.filename_builder import build_profile_filename
from custom_components.cronostar.utils.prefix_normalizer import get_effective_prefix, normalize_preset_type
...
class ProfileService:
    ...
    async def save_profile(self, call: ServiceCall) -> None:
        ...
        try:
            ...
            _LOGGER.info("✅ [SAVE_PROFILE] File saved to storage")

            # 3. Ensure controller entities exist
            await self._ensure_controller_exists(effective_prefix, canonical_preset, meta)

            # 4. Force dashboard update and UI refresh (Atomic Order)
            try:
                from custom_components.cronostar.setup.dashboard import DASHBOARD_YAML_FILENAME, write_dashboard_yaml
                await write_dashboard_yaml(self.hass, DASHBOARD_YAML_FILENAME)
                self.hass.bus.async_fire("lovelace_updated")
                _LOGGER.info("✅ [SAVE_PROFILE] Dashboard updated and UI notified")
            except Exception as e:
                _LOGGER.error("❌ [SAVE_PROFILE] Failed to update dashboard: %s", e)

            # 5. Update existing Config Entry if metadata has changed
#            for entry in self.hass.config_entries.async_entries(DOMAIN):
#                if entry.data.get("global_prefix") == effective_prefix:
#                    # Logic ... (omitted)
#            for entry in self.hass.config_entries.async_entries(DOMAIN):
...
#                        changed = True
    ...
    async def register_card(self, call: ServiceCall) -> ServiceResponse:
        ...
        validation_errors = []
        is_admin = call.data.get("view_mode") == "admin"
